...
public class HtmlDateTimeInput extends ValueWidget<HtmlDateTimeInput, String, ValueDataSourceUsage> implements
        HasType, HasValue<HtmlDateTimeInput, String>, HasKeyInput<HtmlDateTimeInput>, HasRange<String>, HasStep, HasReadonly,
        HasRequired, HasAutocomplete, HasList {
    public HtmlDateTimeInput(WidgetConfigurator widgetConfigurator, String name) {
        super(widgetConfigurator, name, WidgetType.DATETIME, JsType.STRING);
        setType(TypeType.DATETIME_LOCAL);
    }

    public HtmlDateTimeInput() {
        this(NAMELESS, DEFAULT_VALUE_STRING);
    }

    public HtmlDateTimeInput(String name) {
        this(name, DEFAULT_VALUE_STRING);
    }

    public HtmlDateTimeInput(String name, String value) {
        super(name, WidgetType.DATETIME, JsType.STRING);
        setType(TypeType.DATETIME_LOCAL);
        setValue(value);
    }

    public HtmlDateTimeInput addDataSource(DataSourceSupplier dataSourceSupplier, DataAdapter... dataAdapters) throws WidgetConfigurationException {
    ...
    private final KeyInputImpl<HtmlDateTimeInput> keyInputImpl = new KeyInputImpl<>(this);

    @Override
    public HasKeyInput<HtmlDateTimeInput> getKeyInputImpl() {
        return keyInputImpl;
    }

    @Override
    public HtmlDateTimeInput onInput(InputEventHandler eventHandler) {
        return keyInputImpl.onInput(eventHandler);
    }

    @Override
    public HtmlDateTimeInput onKeyDown(KeyDownEventHandler eventHandler) {
        return keyInputImpl.onKeyDown(eventHandler);
    }

    @Override
    public HtmlDateTimeInput onKeyPress(KeyPressEventHandler eventHandler) {
        return keyInputImpl.onKeyPress(eventHandler);
    }

    @Override
    public HtmlDateTimeInput onKeyUp(KeyUpEventHandler eventHandler) {
